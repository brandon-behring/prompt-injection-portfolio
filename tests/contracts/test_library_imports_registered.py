...
primitive consumed from the 4 load-bearing libraries (eval-toolkit /
runpod-deploy / research_toolkit / book-scaffold-astro) must appear as a
row in `decisions/library_imports.md`. Adding a new upstream import without
registering it fails this contract.
...
LOAD_BEARING_LIBS = ("eval_toolkit", "runpod_deploy", "research_toolkit")
...
def _extract_imports(file_path: Path) -> set[str]:
    ...
    imports: set[str] = set()
    pattern = re.compile(
        r"^\s*from\s+(eval_toolkit|runpod_deploy|research_toolkit)"
