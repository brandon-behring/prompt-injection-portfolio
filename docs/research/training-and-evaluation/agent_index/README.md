...
- **"What's the canonical OOD-collapse example?"** → `05_ood_methodology_reproducibility.md` § C5.4 (`protectai2024deberta`) cross-cuts to `03_evaluation_metrics.md` § C3.1 (`jacob2025promptshield` Table 4 reports ProtectAI v2 = 1.34% TPR @ 0.5% FPR and 0.00% TPR @ 0.1% FPR with a no-threshold-exists footnote).
...
- **PINT** (Prompt Injection Test): Lakera 2024 third-party benchmark (3,007 English inputs per the Apr 2024 announcement; deliberately not used as training data); see § C3.4.
...
**Independent audit, round 1 (2026-05-22):** A complementary-scope review pass focused on dataset card freshness + maintainer + license + sample-count drift (C1) and benchmark-validity + OOD-methodology claims (C4 + C5). Findings: 0 dropped, 8 corrected, 3 flagged. Verified verbatim: Saxe F-score quote ("85% F-score on your test data is likely more meaningful…"), 'speed bump, not a wall' framing, Bhagwatkar 'all four public benchmarks' saturation + 'weak attacks, hindering progress', CodeIntegrity post (author Steven, 2026-01-03). Bhagwatkar NeurIPS 2025 acceptance confirmed via NeurIPS virtual page. LLMail-Inject IEEE SaTML 2025 acceptance confirmed via MSRC blog. Major corrections: PromptShield Table 4 reports ProtectAI v2 = **1.34% TPR @ 0.5% FPR / 0.00% TPR @ 0.1% FPR** (NOT 0.00% @ 0.5% as previously written in C3.1 + C5.4 + scope blurb); Lakera PINT input count **3,007** (NOT 4,314); WildJailbreak output is **WildJailbreak (262K pairs)** only — WildGuardMix is the companion WildGuard output (Han 2024). Meta PromptGuard 2 training data is **qualitatively disclosed** (open-source mixtures + synthetic + red-team) — softened C2.5 'undisclosed' claim. AdvBench HF card lists **500** (vs body 520). Recommendation: re-run with focus "training methodology technical claims + GitHub code links".

**Independent audit, round 2 (2026-05-22):** A complementary-scope review pass focused on training-methodology technical claims (C2) + GitHub code links. Prior rounds covered: C1 dataset cards + C4 benchmark validity + C5 OOD methodology. Findings: 0 dropped, 2 corrected, 4 flagged. Verified: Meta PG2 explicitly cites "modified energy-based loss function, inspired by the paper Energy Based Out-of-distribution Detection. In addition to cross-entropy loss, we apply a penalty for large negative energy predictions on benign prompts." DataSentinel IEEE S&P 2025 Distinguished Paper Award confirmed on arXiv comments. LoRA / DoRA / WildGuard / LLMail-Inject GitHub repos all resolve and are official. Major corrections: **Sentinel (Ivry & Nahum 2025) uses ModernBERT-large** (NOT Qwen3-0.6B); LoRA + reflection-style training NOT in abstract — both elements were previously incorrect. Wallace 'context-synthesis' wording clarified as project shorthand, not Wallace et al.'s phrasing (the paper says "data generation method"). HackAPrompt "10 levels of system-prompt difficulty" + "prompts/responses" specifics not in abstract → flagged as unverified body claims. HarmBench "400 behaviors × 7 categories" not in abstract → flagged. JailbreakBench "100 LLMs × adaptive attacks" not in abstract → flagged. AgentDojo 'inverse scaling law' not in abstract → flagged. Recommendation: Clean — two rounds delivered solid OOD-wall load-bearing fixes plus methodology-technical-claim hygiene. Stop here; future rounds should target a different scope (e.g., C3 metric definitions or Topic-B / Topic-D cross-references).
