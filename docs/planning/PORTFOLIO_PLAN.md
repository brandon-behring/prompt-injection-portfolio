...
**Implications**:
- The "carrier is the standing wall" half of the spine is currently **geometric, not a modeling result** (silhouette by-carrier 0.197 vs by-attack-type −0.023; KMeans→carrier ARI 0.98 — `OOD_WALL_PREDICTION/FINDINGS.md`). The carrier-LODO gate converts it to a modeling result or revises the spine — that is the gate's whole purpose; until it runs, prose must name the axis + capacity regime of every "wall" claim. *[2026-06-01 RESOLVED: the gate ran — carrier-LODO verdict SMALL-THROUGHOUT (capacity-attenuated, residual table wall; ADR-055 amendment); this line kept as the original framing. 2026-06-11: the residual table wall is additionally data-resistant at the ceiling (C1 `NOT-CLOSED`; Round 32).]*
...
- **agent-harness-v0 → adopted as a retrospective record trio (Fork B = variant A), placed as its own experiment surface:** `experiments/agent-harness-v0/` enters the record with a `criteria.md` opening RETROSPECTIVE SCOPE DECLARATION (explicitly not a pre-registration), a fenced `FINDINGS.md`, and `verdict.json` = **EXPLORATORY-VALIDATED**. Claim fence: **construction properties of the scripted backend only** — no empirical claims about LLM-agent defense effectiveness (those require a pre-registered v1 with an LLM backend; a P3 option, now unblocked by adoption). **Placement (2026-06-10): own surface** (chapter-7-adjacent), cross-referenced from the Lane 3/4 playbooks (dossier family `agent_harness_architecture`) — a v1 extends it in place.
...
- **Fork C → C1: the Lane-2 carrier/table training arc is the next experiment (decided 2026-06-10).** Attack the one residual within-BIPIA wall (+0.205 table) with carrier-targeted training data — the direct continuation of ADR-055's Lane-2 re-point, with a falsifiable target. **Pre-registration first** (`criteria.DRAFT` → separate present-first ratification before any run); cheap rungs $0 local, `lora` ~$1–5 write-gated behind them. C2 (cross-family mechanism pre-reg) drafts during C1's GPU waits; C3 (agent-harness-v1, LLM backend) stays a P3 option. **Sequencing: P1.5 (methods-hardening FOLLOW-UP package) runs immediately — before the close and before C1** — so the next experiment doesn't build on an unhardened record. *[Executed 2026-06-11: C1 ran end-to-end — verdict `NOT-CLOSED`; see Round 32 below.]*

**Open forks** (registered, not decided):
- **W18** — archival form of the superseded DRAFT-amendment copy in the experiment dir (archive vs delete; user-led — resolves in the P1.5 session; archive-with-banner recommended).
...
- No milestone re-sequencing and no §16 gate changes; M2 = the Lane-2 carrier arc — Fork C decided to C1 above.
...
- **Unchanged / parked:** research_toolkit **PR #38** upstream merge (the repo-local clone already runs the fix — corpus generation was unaffected) *(decided same-day at ratification: merge + BURN_IN_NOTES dogfood entry)*; the **v0.1.0 close stays HELD on accounts** (Fork A(b), Round 31); **DF-11/#93** carrier re-lock.

**Open forks** (registered at drafting; dispositions at the same-day ratification in brackets):
- **ADR-055 carrier-line amendment (the spine fork).** The residual table wall (+0.205 control at `lora`) is now shown **data-resistant at the ceiling**: format-matched, leakage-gated, in-axis training data reduced the frozen rung but did not close — or CI-supportedly reduce — the wall at the `lora` decision rung. Whether the ratified carrier line (**"capacity-attenuated, residual, table-concentrated (provisional, n=3)"**) gains **"…and data-resistant at the ceiling — not closable by targeted same-format synthetic data (n=1 fold, one recipe)"** is a **user ratification fork**, per the amendment-after-verdict precedent (ADR-055's dated 2026-06-01 carrier-LODO + 2026-06-06 cross-family sections). *(**Decided 2026-06-11, same session: TAKEN** — the dated amendment section lands in ADR-055 with the n=1 qualifier; downstream surfaces per the register below.)*
- **C2 ratification** — adjudicate TBD-1..11 + spend scope; a separate session. *(Confirmed 2026-06-11: dedicated present-first session, after this round's fold-in.)*
...
- **No §16 gate changes:** the carrier-study re-test gate (n=3→n=5, qa/abstract-conditional) is unchanged and carries the data-resistance qualifier via the amendment taken above.

**Downstream surfaces for the ADR-055 amendment** (register; each cites the spine/carrier line):
- `decisions/ADR-055-post-m1-re-ladder-multi-axis-spine.md` — the amendment lands here as a new dated section (Decision 1's carrier line + the Carrier-LODO resolution's "honest spine claim" wording).
- `decisions/README.md` — the ADR-055 index row carries the spine summary.
- `docs/planning/PORTFOLIO_PLAN.md` — §9 M1→M2 entry-gate resolution prose, §16 Round-30 gates (carrier-LODO RESOLVED text + carrier-study re-test gate), §5 Lane-2 framing, and the Round-30 update's bracketed refinement annotations.
- `README.md` — milestone table M2+ row ("carrier SMALL-THROUGHOUT … next: Lane-2 carrier training" — the "next" clause is additionally stale now that the Lane-2 carrier-training arc has run).
- `M0_READINESS.md` — the corrected full-spine tag/announcement text quotes the carrier line; the held v0.1.0 close ships the amended wording.
- `docs/glossary.md` — the capacity-attenuated / residual-wall entries.
- `docs/planning/roadmap-refresh-2026-06-09.md` — the STANDING SURFACE "Where the science stands" + spine block + its Fork-C table (C1 was "next"; now resolved).
- `experiments/lane-2/hypothesis.md` + `experiments/lane-2/protocol.md` — the residual wall as Lane-2's live gap (needs the C1 result even without the amendment).
- `docs/planning/portfolio-chapter-outlines.md` — Ch 9 Lane-2 outline + the row-E decision criterion citing the residual table-carrier wall.
- `docs/planning/forward-paths-explained.md` — the plain-language "stubborn gap on table-formatted inputs" passage.
- `experiments/mechanism-style-content/criteria.DRAFT.md` — C1 anchor already refreshed to NOT-CLOSED (2026-06-11).
- `docs/planning/SESSION-HANDOFF.md` / `NEXT_SESSION.md` — prior handoff used "frozen wall REDUCED" informally (F5.1 phrasing correction; the 2026-06-11 PM handoff block carries the corrected phrasing).
...
The three outcomes above are **re-axised to the carrier wall**: M1 showed the *attack-type* wall is **not** structural (capacity dissolves it), so "wall confirmed structural beyond data choice" now refers specifically to the **carrier** wall — whose size is measured *before* Lane 2 commits by the **carrier-LODO M2 pre-flight gate** (§16 Round-30 gates; ADR-055). If that gate finds the carrier gap persists under LoRA, the carrier wall is the live structural question Lane 2 attacks; if LoRA dissolves it too, the spine is revised (capacity dissolves both axes) and Lane 2's headline is resized accordingly. **RESOLVED 2026-06-01 (carrier-LODO M2 pre-flight, ADR-055):** the carrier wall is **measured partially capacity-resistant (not fully structural), residual at the table carrier (provisional, n=3)** — the "structural" pole is partly realized (table, G(lora)=+0.205) / partly dissolved (email/code close at the LoRA ceiling). **First Lane-2 datum 2026-06-11 (C1, Round 32): `NOT-CLOSED`** — the synthetic format-matched table recipe does not close the residual wall at the `lora` ceiling (data-resistant at the ceiling, n=1 fold/one recipe; ADR-055 amendment); the carrier-diverse real-data variant remains the open H-branch frame and the mechanism question feeds C2.
...
Before Lane 2 commits, a **carrier-LODO validation read** sits at the M1-exit → Lane-2-entry boundary (an M2 pre-flight, mirroring the Round-27 EDA-arc-as-M1-entry-gate). It tests whether the multi-axis spine's "carrier is the standing wall" claim — geometric so far (silhouette by-carrier 0.197 vs by-attack-type −0.023) — survives end-to-end LoRA, and sizes Lane 2's scope. Registered in §16 (Round-30 gates) + ADR-055; criteria pre-registered at `experiments/carrier-lodo/criteria.md`. Milestone *order* is unchanged (still M0→M7; no rung added). **RESOLVED 2026-06-01 — `SMALL-THROUGHOUT`** (ADR-055 Carrier-LODO resolution): the carrier gap is real at frozen (G=+0.167), capacity-attenuated at the LoRA ceiling (G=+0.067, CI-low +0.064 > 0 but < ½·G(frozen)), with a residual table-carrier wall (+0.205) — partially capacity-resistant (provisional, n=3), not a fully standing wall. **The C1 follow-on (2026-06-11, Round 32) then showed that residual table wall is data-resistant at the ceiling** (targeted same-format synthetic data: `NOT-CLOSED`; ADR-055 Carrier-table training resolution).
