...
| MR-13 | brandon-behring/book-scaffold-astro | [#54](https://github.com/brandon-behring/book-scaffold-astro/issues/54) | `npm run build:bib` (citation-js plugin-bibtex) fails when the canonical `bibliography.bib` template ships with a commented-out `% @article{example-key2024, ...}` example block. citation-js's lexer treats `@TYPE` tokens inside `%`-comments as real entry starts. Surfaced during M0 close Phase I (Round 24) when the portfolio's 157-entry bibliography.bib + template header tripped the parser. v3.6.2-3.6.4 already addressed adjacent `@<word>-token + Cite-key` template fixes but missed this case. | Portfolio's `book/bibliography.bib` header was edited to remove the `% @article{...}` template block (prose-only header preserved); section sign `§` retained in entry titles. `npm run build:bib` + `npm run build:labels` + `npm run validate` all pass after fix. | issue-filed 2026-05-23 via Phase I; suggested upstream fix: (a) drop the commented-out example from create-book template, OR (b) wrap example in `@comment{...}` form. Targets book-scaffold-astro v3.6.6+ or v3.7.0. NOT blocking — portfolio workaround sustainable. |
| MR-14 | brandon-behring/research_toolkit | [#14](https://github.com/brandon-behring/research_toolkit/issues/14) | v2.3.0-candidate `cache_manifest.py` REJECTS absolute / `~`-prefixed paths + points at `scripts/migrate_manifest_paths.py`. After migration, `v2_common.py:112-114` (used by evidence_ledger validator) still resolves relative `text_path` via `manifest_path.parent` (the dossier directory) instead of via `cache_root` from the manifest header. Both validators inconsistent: pre-migration FAILS cache_manifest.py (216 errors/topic); post-migration FAILS v2_common.py (61 errors/topic on evidence_ledger). No intermediate state passes both. Surfaced during M0 close Round 25 Phase 4 (post-v2.3-candidate upstream commit `daf6699`); `make dossier-audit` passed under v2.2.0 yesterday but fails today. | Portfolio reverted the migration script's edits + accepted the pre-v2.3 baseline (`make dossier-audit` reports cache_manifest.py path-portability errors but the dossier itself is unchanged from Round 24 Phase G PASS). Tested both states — no portfolio-side workaround sustainable. | issue-filed 2026-05-23 via Phase 4; suggested upstream fix: pass `cache_root` parameter to `v2_common.verify_excerpt_anchor()` + resolve relative `text_path` against expanded `cache_root` instead of `manifest_path.parent`. Alternatively share `_resolve()` logic between cache_manifest.py + v2_common.py. Targets research_toolkit v2.3.x or v2.4.0. NOT blocking immediate work (dossier state unchanged) but blocks `make dossier-audit` close-gate. |
